...
view model =
    let
        configMarkup =
            case model.mode of
                Config ->
                    configPanel model.configuration model.configPanel
...
buildRepositoryListing statuses =
    let
        listElems =
            List.map asListItem statuses
    in
    H.ul [ A.class "branch-list" ] listElems


asListItem : RadiatorStatus -> Html Msg
asListItem s =
    H.li [ A.class ("branch " ++ s.state) ] (branchElems s)
...
branchElems { repository, branch } =
    let
        repoName =
            displayableRepoName repository

        branchName =
            Maybe.withDefault repoName (Maybe.map ((++) (repoName ++ ": ")) branch)
    in
    [ H.span [ A.class "branch-name" ] [ H.text branchName ] ]
...
configPanel { repositories, apiKey } { repositorySlug, apiKeyValue } =
    let
        ...
        repositoryItems =
            List.map repositoryItem repositories
    in
    [ H.div [ A.class "config-panel" ]
        [ H.button
            [ A.class "config-close-button"
...
usePrivateTravisInput private =
    H.div [ A.class "config-panel-control-row" ]
        [ H.input
            [ A.id "use-private-travis-checkbox"
            , A.type_ "checkbox"
            , A.checked private
...
displayableRepoName name =
    let
        nameParts =
            String.split "/" name
    in
    if List.length nameParts > 1 then
        List.drop 1 nameParts
            |> List.head
