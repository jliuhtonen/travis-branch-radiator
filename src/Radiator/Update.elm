module Radiator.Update exposing (refreshBuilds, update)
    ...
    case action of
        ...
        UpdateRepositoryField repository ->
            let
                ...
                configView =
                    { cfg | repositorySlug = repository }
            in
            ( { model | configPanel = configView }, Cmd.none )
        ...
        AddRepository ->
            let
                ...
                updatedRepositories =
                    List.append model.configuration.repositories [ model.configPanel.repositorySlug ]

                updatedModel =
                    { model | configuration = { currentConfig | repositories = updatedRepositories }, configPanel = { cfgPanel | repositorySlug = "" } }
            in
            ( updatedModel, updateConfigurationCmd updatedModel.configuration )

        RemoveRepository repository ->
            let
                newRepositories =
                    List.filter (\r -> r /= repository) model.configuration.repositories
            in
            updateConfig (\cfg -> { cfg | repositories = newRepositories }) model

        TogglePrivateTravis usePrivateTravis ->
            let
                newApiKey =
                    if usePrivateTravis then
                        Just model.configPanel.apiKeyValue

                    else
                        Nothing
            in
            updateConfig (\cfg -> { cfg | apiKey = newApiKey }) model
...
refreshModelBuildState newStatus model =
    let
        newBuildStatuses =
            toRadiatorStatusList newStatus
        ...
        updatedBuildStatus =
            List.concatMap (\r -> List.filter (\s -> s.repository == r) radiatorStatuses) model.configuration.repositories
    in
    { model | buildStatus = updatedBuildStatus }
...
toRadiatorStatusList ( repository, branchStatuses ) =
    let
        nonPassed =
            List.filter (\branch -> branch.lastBuild.state /= "passed" && branch.lastBuild.state /= "canceled") branchStatuses
    in
    case nonPassed of
        [] ->
        ...
        xs ->
            List.map (\branch -> RadiatorStatus repository (Just branch.name) branch.lastBuild.state) nonPassed
